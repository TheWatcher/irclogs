## @file
# This file contains the IRCLogs-specific implementation of the runtime
...
# IRCLogs application.
package IRCLogs::System;

use strict;
use base qw(Webperl::System);

use IRCLogs::System::Metadata;
use IRCLogs::System::Roles;
use IRCLogs::System::Tags;

## @method $ init(%args)
# Initialise the IRCLogs System's references to other system objects. This
# sets up the IRCLogs-specific modules, placing references to them into the
...
sub init {
    ...
    # now create the IRCLogs-specific objects
    $self -> {"metadata"} = IRCLogs::System::Metadata -> new(dbh      => $self -> {"dbh"},
                                                             settings => $self -> {"settings"},
                                                             logger   => $self -> {"logger"})
        or return $self -> self_error("Metadata system init failed: ".$Webperl::SystemModule::errstr);

    $self -> {"tags"} = IRCLogs::System::Tags -> new(dbh      => $self -> {"dbh"},
                                                     settings => $self -> {"settings"},
                                                     logger   => $self -> {"logger"},
                                                     metadata => $self -> {"metadata"})
        or return $self -> self_error("Tag system init failed: ".$Webperl::SystemModule::errstr);

    $self -> {"roles"} = IRCLogs::System::Roles -> new(dbh      => $self -> {"dbh"},
                                                       settings => $self -> {"settings"},
                                                       logger   => $self -> {"logger"},
                                                       metadata => $self -> {"metadata"})
